...
def mutFlipSign(indiv):
  """Flip the sign of the attributes of the input individual and return the
  mutant. The *individual* is expected to be a :term:`sequence` and the values of the
  attributes shall stay valid after the ``flip`` operator is called on them.
  The *indpb* argument is the probability of each attribute to be
  flipped. This mutation is usually applied on signed descrete value individuals.

  :param indiv: Individual to be mutated.
  :returns: An individual.

  Function re-written from the mutFlipByte original DEAP function and now is NOT using
  a for loop.
  This function uses the :func:`~random.random` function from the python base
  :mod:`random` module.
  """
  i = random.randrange(0, len(indiv))
  indiv[i] = indiv[i]*-1
  return(indiv)
...
__all__ = ['mutFlipSign', 'mutGaussian', 'mutPolynomialBounded', 'mutShuffleIndexes',
